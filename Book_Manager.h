...
// Coded by Atharva Joshi(@A.J_1331)
...
// This function checks if the text file containing the book records exists or not
// if it does not then it creates a blank text file for books
void BookManager::checkBookEnv()
{
    FILE *fp = fopen ("AllBooks.txt", "r"); 		// These functions are written in C... 
    if (fp!=NULL)					//They are written in C just to reduce the coding time.
        fclose (fp);
    else
        createBlankFile();
}
...
void BookManager::readBookInfo() //Read all book information from file to BookList
{
    ...
    while (fgets(buff, 113, fp)!=NULL)
    {
            ...
            //cout<< BookList[i].get_book_id()<<endl;			//These are alternate codes to the previous lines. 
...
void BookManager::addBookInfo()
{
   ...
   cin.getline (cbuff,sizeof(cbuff)); // dummy getline command to get it work
    ...
    //cin.getline (bs.book_author_name,sizeof(bs.book_author_name));				//ALTERNATIVE
    cin.getline (cbuff,sizeof(cbuff));
...
// This function will print a report based on given category(coded in C).
// The report is not properly aligned.
